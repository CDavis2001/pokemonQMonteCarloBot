...
class QLearningLiteDemoPlayer(Player):
    ...
    def choose_move(self, battle):
        ...
        observation = QLearningLiteDemoPlayer.embed_battle(battle)
        # ------------------------------------------------------------------
           ...
           self.firstturn = False 
            ...
            try:
                ...
                if prev_state == None:
                    ...
                    log("updating state from " + oldreward + " to " + newreward)
        ...
        for i in range(len(KB["KB"])):
            ...
            if KB["KB"][i]["observation"] == observation:
                log("Found Previous Observation")
                memory = KB["KB"][i]
                new = False
        ...
        if new:
            ...
            action = actions[choice]["action"]
            action = action.split(";")
            log("Choosing Random Move in New State")
            if action[1] == "switch":
                action = Pokemon(species=action[0])
            else:
                action = Move(move_id = action[0])
        ...
        # Dealing with familiar state
            ...
            if choice == None:
                ...
                log("Chosen Move with highest utility")
                action = choice.split(";")
                if action[1] == "switch":
                    action = Pokemon(species=action[0])
